...
//  SwiftCamera
//
//  Created by Daesik Choi on 3/20/21.
//
...
public class CameraService {
    ...
    //a spinner indicating that work is going on to process the captured photo.
    ...
    // 11. Stores whether the session is been configured or not.
    ...
    public func checkForPermissions() {
        ...
        default:
            ...
            DispatchQueue.main.async {
                self.alertError = AlertError(title: "Camera Access", message: "SwiftCamera doesn't have access to use your camera, please update your privacy settings.", primaryButtonTitle: "Settings", secondaryButtonTitle: nil, primaryAction: {UIApplication.shared.open(URL(string: UIApplication.openSettingsURLString)!, options: [:], completionHandler: nil)}, secondaryAction: nil)
    ...
    // A video capture session runs independently from the object that declares the dependency on it. That means if we were to have a view with a camera preview and attached to it out CameraService, and then we were to close this view and de-initialize CameraService, the camera capture session will keep running.
    ...
    public func changeCamera() {
        ...
        sessionQueue.async {
            ...
            @unknown default:
                print("Unknown capture session. Defaulting to back, dual-camera.")
                preferredPosition = .back
                preferredDeviceType = .builtInWideAngleCamera
            }
